...
class Step01JjwtLibTest {
    ...
    void jjwtLibQuickstart() {
        ...
        String jws = Jwts.builder().subject("Joe") // 일반적으로 아이디
                .claim("role", "admin")
                .expiration(Date
                        .from(LocalDateTime.now().plusHours(1).toInstant(ZoneOffset.of("+09:00"))))
                .issuedAt(Date.from(LocalDateTime.now().toInstant(ZoneOffset.of("+09:00"))))
                .signWith(key).compact();

        log.info(jws);

        Jws<Claims> claims = Jwts.parser().verifyWith(key).build().parseSignedClaims(jws);
