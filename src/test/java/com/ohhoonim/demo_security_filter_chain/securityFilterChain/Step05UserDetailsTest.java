...
public class Step05UserDetailsTest {
    ...
    public void entpointTest() {
        mockMvcTester.get().uri("/main").assertThat().hasStatus(200);
    ...
    public static class SecurityConfig {
        ...
        @Bean
        UserDetailsService userDetailsService() {
            return new InMemoryUserDetailsManager(User.withUsername("ohhoonim")
                    .password(passwordEncoder().encode("password")).roles("USERB") // roles 또는 authorities 중 하나만 써야함. 나중에 적용한것이 덮어씀
                    .authorities("TEAM_A", "ROLE_USER").build());
        }
    }
}
